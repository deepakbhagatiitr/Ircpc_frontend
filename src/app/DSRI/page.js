...
export default function ViewPatent() {
    ...
    const postComment = async (patentId) => {
        try {
            await axios.post(
                `https://iprc-backend-208970416432.us-central1.run.app/api/profiles/patents/${patentId}/comment`,
    ...
    const fetchPatentDetails = async (patentId) => {
        try {
            const response = await axios.get(
                `https://iprc-backend-208970416432.us-central1.run.app/api/profiles/patent/${patentId}`
            );
    ...
    const approvePatent = async (patentId) => {
        try {
            setLoading(true)
            await axios.put(
                `https://iprc-backend-208970416432.us-central1.run.app/api/profiles/DSRI/patents/${patentId}/approve`
            );
            setLoading(false)
            setApproved(true);
            setRejected(false);
    ...
    const rejectPatent = async (patentId) => {
        try {
            ...
            await axios.put(
                `https://iprc-backend-208970416432.us-central1.run.app/api/profiles/patents/${patentId}/reject`
            );
            setLoading(false)
            setApproved(false);
            setRejected(true);
